...
Result utils::walkDirectory(std::string const& root,
    ...
    R_TRY(RESULT_CODE(nn::fs::OpenDirectory(&rootHandle, root.c_str(), nn::fs::OpenDirectoryMode_All)));

    s64 entryCount;
    r = RESULT_CODE(nn::fs::GetDirectoryEntryCount(&entryCount, rootHandle));
    if (R_FAILED(r)) {
    ...
    r = RESULT_CODE(nn::fs::ReadDirectory(&entryCount, entryBuffer, rootHandle, entryCount));
    nn::fs::CloseDirectory(rootHandle);
...
Result utils::readEntireFile(std::string const& str, void** dataptr, u64* length) {
    ...
    R_TRY(RESULT_CODE(nn::fs::OpenFile(&handle, str.c_str(), nn::fs::OpenMode_Read)));

    s64 size;
    nn::Result r = nn::fs::GetFileSize(&size, handle);
...
Result utils::writeFile(std::string const& str, s64 offset, void* data, size_t length) {
    ...
    if (RESULT_CODE(rc) == 0x202) {  // Path does not exist
        R_TRY(RESULT_CODE(nn::fs::CreateFile(str.c_str(), offset + length)));
    } else if (R_FAILED(RESULT_CODE(rc)))
        return RESULT_CODE(rc);

    if (entryType == nn::fs::DirectoryEntryType_Directory) return -1;

    nn::fs::FileHandle handle;
    R_TRY(RESULT_CODE(nn::fs::OpenFile(&handle, str.c_str(), nn::fs::OpenMode_ReadWrite | nn::fs::OpenMode_Append)));
...
Result utils::entryCount(u64* out, std::string const& path, nn::fs::DirectoryEntryType entryType) {
    ...
    R_TRY(RESULT_CODE(nn::fs::GetEntryType(&pathType, path.c_str())));
